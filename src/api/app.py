...
REQUIRED_FEATURES = [
    'bedrooms', 'bathrooms', 'sqm_living', 'sqm_lot', 'floors', 
    'waterfront', 'view', 'condition', 'grade', 'yr_built', 
    'yr_renovated', 'lat', 'long', 'sqm_living15', 'sqm_lot15', 
    'basement_ratio', 'year', 'month', 'day', 'dayofweek'
]
...
class PredictionPayload(BaseModel):
    data: dict = Field(..., example={"id": "7229300521", "bedrooms": 2, "sqft_living": 1180})
...
async def predict(payload: PredictionPayload):
    if model is None:
        raise HTTPException(status_code=500, detail="Model not loaded. Check MODEL_PATH and IAM permissions.")
